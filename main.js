const { app, BrowserWindow, ipcMain, screen, globalShortcut, desktopCapturer, dialog, shell } = require('electron')
...
const priorityFolders = ['xiaoai', 'Hiyouri', 'Default', 'Main'];
...
ipcMain.handle('open-web-ui', async () => {
    try {
        let port = '5001'
        const portFile = path.join(app.getAppPath(), 'data', 'webui_port.txt')
        if (fs.existsSync(portFile)) {
            const raw = fs.readFileSync(portFile, 'utf-8').trim()
            const n = parseInt(raw, 10)
            if (raw && /^\d+$/.test(raw) && n > 0 && n <= 65535) {
                port = String(n)
            }
        }
        ...
        console.error('open-web-ui 失败:', error)
...
/** 解析可用的 VRM 路径：优先配置中已有且文件存在，否则扫描 3D 目录，最后回退默认名 */
function resolveVrmModelPath(configData, appPath) {
    const tryPath = (rel) => {
        if (!rel) return null;
        const abs = path.join(appPath, rel.replace(/\//g, path.sep))
        return fs.existsSync(abs) ? rel.replace(/\\/g, '/') : null
    }
    const fromConfig = tryPath(configData.ui?.vrm_model_path)
    if (fromConfig) return fromConfig
    const dir3d = path.join(appPath, '3D')
    if (fs.existsSync(dir3d)) {
        const files = fs.readdirSync(dir3d).filter((f) => /\.vrm$/i.test(f))
        if (files.length) return `3D/${files[0]}`
    }
    return '3D/Fioka1.vrm'
}

// Live2D ↔ VRM 一键切换（写 config 后整窗 reload）
ipcMain.handle('switch-model-renderer', async (event, payload) => {
    const target = payload && payload.target
    if (target !== 'live2d' && target !== 'vrm') {
        return { success: false, message: '无效的渲染模式' }
    }
    try {
        const configData = JSON.parse(fs.readFileSync(configPath, 'utf8'))
        ...
        if (target === 'live2d') {
            ...
            const win = BrowserWindow.fromWebContents(event.sender)
            win.reload()
            return { success: true, message: '已切换到 Live2D，窗口将重新加载' }
        }

        const vrmRel = resolveVrmModelPath(configData, app.getAppPath())
        const fileName = path.basename(vrmRel)
        configData.ui.model_type = 'vrm'
        configData.ui.vrm_model_path = vrmRel
        configData.ui.vrm_model = fileName
        fs.writeFileSync(configPath, JSON.stringify(configData, null, 2), 'utf8')

        const win = BrowserWindow.fromWebContents(event.sender)
        win.reload()
        return { success: true, message: `已切换到 VRM（${fileName}），窗口将重新加载` }
    } catch (error) {
...
ipcMain.handle('switch-live2d-model', async (event, modelName) => {
    try {
        ...
        try {
            ...
            console.error('同步 config model_type=live2d 失败:', cfgErr)
        }
