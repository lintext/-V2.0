...
function renderChatHistory(messages, prependToTop = false, scrollBeforeLoad = 0, scrollHeightBeforeLoad = 0) {
    ...
    if (!messages || messages.length === 0) {
        ...
        container.innerHTML = `
            <div class="chat-empty">
                ...
                <div class="chat-empty-text">暂无对话记录</div>
                <div class="chat-empty-hint">开始与小艾同学对话吧</div>
            </div>
        `;
        return;
    }
    ...
    htmlParts.push(`
        <div class="chat-header">
            <div class="chat-header-title">
                <span>💬 对话历史</span>
                <span class="count">${chatHistoryState.total} 条</span>
            </div>
        </div>
        <div class="chat-messages-wrapper">
            <div class="chat-load-more" id="chat-load-more-container">
                <button id="chat-load-more-btn" onclick="loadMoreChatHistory()" ${!chatHistoryState.hasMorePrev ? 'disabled' : ''}>
                    ${chatHistoryState.hasMorePrev ? '📜 加载更多历史对话' : '✓ 已加载全部对话'}
                </button>
            </div>
    `);
    ...
    messages.forEach((msg) => {
        ...
        const senderName = role === 'user' ? '用户' : '小艾';
        const avatarText = role === 'user' ? '👤' : '🤖';
        const timeStr = msg.created_at ? formatChatTime(msg.created_at) : '';
            ...
            if (Array.isArray(msg.content)) {
                msg.content.forEach(item => {
                    if (item.type === 'text') {
                        ...
                        contentHtml += `<img src="${imageUrl}" alt="图片" onclick="previewImage(this.src)">`;
                    }
                });
            } else {
    ...
    const messagesWrapper = container.querySelector('.chat-messages-wrapper');
    const scrollContainer = messagesWrapper || container;

    setTimeout(() => {
        if (prependToTop) {
            const newScrollHeight = scrollContainer.scrollHeight;
            const heightDiff = newScrollHeight - scrollHeightBeforeLoad;
            scrollContainer.scrollTop = scrollBeforeLoad + heightDiff;
            console.log(`[ChatHistory] 加载更多，恢复滚动位置：${scrollContainer.scrollTop}`);
        } else {
            const targetScroll = scrollContainer.scrollHeight;
            scrollContainer.scrollTop = targetScroll;
            console.log(`[ChatHistory] 滚动到底部：${targetScroll}, 实际滚动位置：${scrollContainer.scrollTop}`);
            
            setTimeout(() => {
                console.log(`[ChatHistory] 验证滚动位置：${scrollContainer.scrollTop} / ${scrollContainer.scrollHeight}`);
            }, 200);
        }
    }, 300);
}
...
// 在桌面应用中打开趣味面板
function openFunPanelInApp() {
    fetch('/api/fun-panel/open')
        .then(response => response.json())
        .then(data => {
            if (data.success) {
                showToast('已在桌面打开趣味面板！', 'success');
            } else {
                showToast('打开趣味面板失败：' + (data.error || '未知错误'), 'error');
            }
        })
        .catch(error => {
            showToast('打开趣味面板时出错：' + error.message, 'error');
        });
}

// 切换角色
function switchGenderWebUI(gender, btnElement) {
    // 更新按钮状态
    document.querySelectorAll('.gender-btn-webui').forEach(btn => {
        btn.classList.remove('active');
    });
    if (btnElement) {
    ...
    // 发送请求到后端
    fetch('/api/fun-panel/switch-gender', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        ...
        .then(data => {
            if (data.success) {
                showToast('已切换为' + (gender === 'female' ? '小艾' : '小阳') + '！', 'success');
            } else {
                showToast('切换角色失败：' + (data.error || '未知错误'), 'error');
            }
        })
        .catch(error => {
            showToast('切换角色时出错：' + error.message, 'error');
        });
}

// 换装
function changeCostumeWebUI(costumeId, btnElement) {
    // 更新按钮状态
    document.querySelectorAll('.costume-btn-webui').forEach(btn => {
        btn.classList.remove('active');
    });
    if (btnElement) {
        btnElement.classList.add('active');
    ...
    // 发送请求到后端
    fetch('/api/fun-panel/change-costume', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        ...
        .then(data => {
            if (data.success) {
                showToast('换装成功！', 'success');
            } else {
                showToast('换装失败：' + (data.error || '未知错误'), 'error');
            }
        })
        .catch(error => {
            showToast('换装时出错：' + error.message, 'error');
        });
}

// 触发动作
function triggerActionWebUI(actionId) {
    const actionMap = {
        'wave': 'Hiyori_m01',
        'happy': 'Hiyori_m02',
        'think': 'Hiyori_m03',
    ...
    const motionName = actionMap[actionId] || actionId;
    
    fetch('/api/fun-panel/play-action', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        ...
        .then(data => {
            if (data.success) {
                showToast('动作已触发！', 'success');
            } else {
                showToast('触发动作失败：' + (data.error || '未知错误'), 'error');
            }
        })
        .catch(error => {
            showToast('触发动作时出错：' + error.message, 'error');
        });
}

// 触发表情
function triggerExpressionWebUI(expressionId) {
    fetch('/api/fun-panel/set-expression', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ expression: expressionId })
    })
        .then(response => response.json())
        .then(data => {
            if (data.success) {
                showToast('表情已设置！', 'success');
            } else {
                showToast('设置表情失败：' + (data.error || '未知错误'), 'error');
            }
        })
        .catch(error => {
            showToast('设置表情时出错：' + error.message, 'error');
        });
}

// 触发特效
function triggerEffectWebUI(effectId) {
    fetch('/api/fun-panel/play-effect', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        ...
        .then(data => {
            if (data.success) {
                showToast('特效已触发！', 'success');
            } else {
                showToast('触发特效失败：' + (data.error || '未知错误'), 'error');
            }
        })
        .catch(error => {
            showToast('触发特效时出错：' + error.message, 'error');
        });
}

// 触发快捷互动
function triggerQuickInteractWebUI(interactType) {
    fetch('/api/fun-panel/quick-interact', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ interact: interactType })
    })
        .then(response => response.json())
        .then(data => {
            if (data.success) {
                showToast('互动已触发！', 'success');
            } else {
                showToast('触发互动失败：' + (data.error || '未知错误'), 'error');
            }
        })
        .catch(error => {
            showToast('触发互动时出错：' + error.message, 'error');
        });
}
...
let availableModels = { live2d: [], vrm: [] };
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (data.success) {
                ...
                updateLive2DModelSelect();
                updateVRMModelSelect();
...
function updateLive2DModelSelect() {
    const select = document.getElementById('live2d-model-select');
    if (!select) return;
    
    select.innerHTML = '';
    availableModels.live2d.forEach(model => {
        const option = document.createElement('option');
...
function updateVRMModelSelect() {
    const select = document.getElementById('vrm-model-select');
    if (!select) return;
    
    select.innerHTML = '';
    availableModels.vrm.forEach(model => {
        const option = document.createElement('option');
        option.value = model.name;
        option.textContent = model.name;
        option.dataset.path = model.path;
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (data.success && data.current) {
                ...
                const live2dSelect = document.getElementById('live2d-model-select');
                const vrmSelect = document.getElementById('vrm-model-select');
                
                if (typeSelect) {
                    typeSelect.value = data.current.type || 'live2d';
                    onModelTypeChange();
                }
                
                if (data.current.type === 'vrm' && vrmSelect) {
                    vrmSelect.value = data.current.name;
                } else if (live2dSelect) {
                    live2dSelect.value = data.current.name;
                }
            }
        }
    } catch (error) {
        console.error('加载当前模型信息失败:', error);
    }
}

function onModelTypeChange() {
    const typeSelect = document.getElementById('model-type-select');
    const live2dSection = document.getElementById('live2d-model-section');
    const vrmSection = document.getElementById('vrm-model-section');
    ...
    if (live2dSection) {
        live2dSection.style.display = selectedType === 'live2d' ? 'block' : 'none';
    }
    if (vrmSection) {
        vrmSection.style.display = selectedType === 'vrm' ? 'block' : 'none';
    }
}

async function saveVRMModel() {
    try {
        const vrmSelect = document.getElementById('vrm-model-select');
        const modelName = vrmSelect.value;
        const selectedOption = vrmSelect.selectedOptions[0];
        const modelPath = selectedOption ? selectedOption.dataset.path : '';
        ...
        if (response.ok && result.success) {
            showSuccess(result.message || `已切换到VRM模型：${modelName}`);
            addLog(`VRM模型已切换为：${modelName}`, 'success', 'system');
        } else {
            showError('切换失败：' + (result.error || '未知错误'));
        }
    } catch (error) {
        showError('切换模型时出错：' + error.message);
    }
}

async function toggleModelVisibility(visible) {
    try {
        ...
        if (response.ok && result.success) {
            showSuccess(result.message || (visible ? '模型已显示' : '模型已隐藏'));
        } else {
            showError('操作失败：' + (result.error || '未知错误'));
        }
    } catch (error) {
        showError('操作出错：' + error.message);
    }
}

async function refreshModelState() {
    try {
        const response = await fetch('/api/models/state');
        if (response.ok) {
            const data = await response.json();
            if (data.success && data.state) {
                showInfo(`模型状态：${data.state.type} - ${data.state.name || '未知'}`);
            }
        }
    } catch (error) {
...
async function playModelMotion(motionName) {
    try {
        const response = await fetch('/api/models/motion', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ motion: motionName })
        });
        ...
        const result = await response.json();
        if (result.success) {
            showSuccess(`动作已播放：${motionName}`);
        }
    } catch (error) {
        showError('播放动作失败：' + error.message);
    }
}

async function setModelExpression(expressionName) {
    try {
        const response = await fetch('/api/models/expression', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        const result = await response.json();
        if (result.success) {
            showSuccess(`表情已设置：${expressionName}`);
        }
    } catch (error) {
        showError('设置表情失败：' + error.message);
    }
}
